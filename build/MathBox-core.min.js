...
m=b.tickScale,b=b.decimals,l=this.points,r=this.labelPoints,q=this.labelTangent,n=this.tickPoints,s=this.tickSigns,t=[0,0,0],u=new THREE.Vector3;new THREE.Vector4;a=a.axis(c);var v=a[0];a=a[1];var x=(a-v)/(h-1);u.set.apply(u,d);_.loop(h,function(a){t[c]=v+a*x;l[a].set.apply(l[a],t);l[a].addSelf(u)});this.line.show(f);this.arrow.show(e);h=this.scale=MathBox.Ticks(v,a,g,k,m,!0);this.ticks.show(!!g);if(g){g=Math.floor(n.length/2);h.length>g&&(h=h.slice(0,g));var g=[0,0,0],w;g[2==c?0:1-c]=0.01*(a-v);
this.epsilon.set.apply(this.epsilon,g);t=[0,0,0];_.each(h,function(a,b){t[c]=a;var d=2*b;n[d].set.apply(n[d],t);n[d].addSelf(u);s[d]=1;n[d+1].copy(n[d]);s[d+1]=-1;r[b].copy(n[d]);w=d+1});g=w+1;for(h=n.length;g<h;)n[g].copy(n[w]),s[g]=s[w],g++}t=[0,0,0];t[c]=1;q.set.apply(q,t);this.labels.show(!!j);this.labels.set("decimals",b)},make:function(){var a=this.get(),b=a.n,c=a.size,d=a.ticks,e=a.distance,f=a.formatter,j=this.style,h=this.points=[],g=this.labelPoints=[],k=this.labelTangent=new THREE.Vector3,
...
MathBox.Vector.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{n:1,data:null,arrow:!0,expression:function(){return 0},live:!0,style:{},size:0.07}},type:function(){return"vector"},renderables:function(){return this.render},adjust:function(a){var b=this.get();this.calculate(a);_.each(this.arrows,function(a){a.show(b.arrow)})},make:function(){var a=this.get(),b=a.arrow,c=this.style,d=a.n,e=this.vertices=[],f=this.points=[],j=this.render=[],h=this.arrows=[],g={dynamic:a.live,
type:"line",strip:!1},k={size:a.size},m=0;_.loop(d,function(){e.push(new THREE.Vector3);e.push(new THREE.Vector3);f.push(new THREE.Vector3);f.push(new THREE.Vector3);if(b){var a=new MathBox.Renderable.ArrowHead(f[m++],f[m++],k,c);j.push(a);h.push(a)}});this.line=new MathBox.Renderable.Mesh(e,g,c);j.push(this.line);this.calculate()},calculate:function(a){var b=this.vertices,c=this.arrows,d=this.points,e=this.get(),f=e.data,j=e.arrow,h=e.n,g=e.size,k=this.style.get("mathScale"),m=new THREE.Vector3,
l=new THREE.Vector3,r=0,q=0;_.loop(2*h,function(h){p=f&&void 0!==f[h]?f[h]:e.expression.call(this,q,r);p instanceof Array||(p=[h,p,0]);p=p.concat([0,0,0]);d[h].set.apply(d[h],p);b[h].set.apply(b[h],p);if(a&&j&&1==h%2){l.copy(b[h]);m.copy(b[h-1]);a.to(l);a.to(m);l.subSelf(m).multiplySelf(k);var s=l.length(),t=Math.min(1,0.5*s/g),t=(1-(1-t)*(1-t))*g;l.normalize().multiplyScalar(s-t).divideSelf(k).addSelf(m);a.from(l);b[h].copy(l);c[q].set({size:t})}q=r?q+1:q;r=(r+1)%2}.bind(this))}});
...
[[],[]];_.loop(c[0]*c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,j=c.flipSided,h=j?1:0,j=j?0:1;"number"==typeof f&&(f=[f,f]);var g=e[0][0],k=e[1][0],m=(e[0][1]-g)/(f[0]-1),l=(e[1][1]-k)/(f[1]-1),r,q=0;_.loop(f[1],function(b){g=e[0][0];_.loop(f[0],function(e){r=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,g,k,e,b,q);r instanceof Array||
(r=[g,r,k]);r=r.concat([0,0,0]);a[q].set.apply(a[q],r);g+=m;q++}.bind(this));k+=l}.bind(this));if(c.shaded){var q=0,k=e[1][0],n=f[0];_.loop(f[1],function(c){g=e[0][0];var d=c?c-1:0,h=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var j=e?e-1:0,k=Math.min(f[0]-1,e+1),l=a[e+c*n];k==e?b[0][q].sub(l,a[j+c*n]).addSelf(l):b[0][q].copy(a[k+c*n]);h==c?b[1][q].sub(l,a[e+d*n]).addSelf(l):b[1][q].copy(a[e+h*n]);g+=m;q++});k+=l});this.line.set("attributes",{positionDU:b[h],positionDV:b[j]});this.mesh.set("attributes",
...
1),n=0;_.loop(j[1],function(){m=c[0][0];_.loop(j[0],function(){a[n].set(m,l,0);m+=r;n++}.bind(this));l+=q}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});
...
d.className="mathbox-wrap";d.style.position="relative";d.style.display="inline-block";d.style.left="-50%";d.style.top="-.5em";b.push(h);e.add(h)})},adjust:function(a,b,c,d,e){var f=this.get(),j=this.points,h=this.callback,g=f.decimals,k=this.style,m=window.MathJax&&MathJax.Hub;_.each(this.sprites,function(b,c){b.position.copy(j[c]);a.to(b.position);e.matrix.multiplyVector3(b.position);b.distance=f.distance;b.element.style.opacity=k.get("opacity");var d="";if(h){d=h(c);void 0===d&&(d="");if(+d==d){var n=
+d;if(0!=n)var d=0>n?-1:1,n=Math.abs(n),s=Math.pow(10,g-1-Math.floor(Math.log(n)/Math.log(10))),d=n=d*Math.round(s*n)/s}m||(d=(""+d).replace(/^-/,"\u2013"))}b.content!==d&&(n=b.element.children[0],b.content=d,b.measure=!0,m?(n.innerHTML="\\("+d+"\\)",MathJax.Hub.queue.Push(["Typeset",MathJax.Hub,n])):n.innerHTML=d)});MathBox.Renderable.prototype.adjust.call(this,a)}});
