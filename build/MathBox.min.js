...
strip:!1},j={size:a.size},k=0;_.loop(c,function(){d.push(new THREE.Vector3);d.push(new THREE.Vector3);e.push(new THREE.Vector3);e.push(new THREE.Vector3);var a=new MathBox.Renderable.ArrowHead(e[k++],e[k++],j,b);f.push(a);g.push(a)});this.line=new MathBox.Renderable.Mesh(d,h,b);f.push(this.line);this.calculate()},calculate:function(a){var b=this.vertices,c=this.arrows,d=this.points,e=this.get(),f=e.data,g=e.arrow,h=e.n,j=e.size,k=new THREE.Vector3,l=new THREE.Vector3,m=0,r=0;_.loop(2*h,function(h){p=
f&&void 0!==f[h]?f[h]:e.expression.call(this,r,m);p instanceof Array||(p=[h,p,0]);p=p.concat([0,0,0]);d[h].set.apply(d[h],p);b[h].set.apply(b[h],p);if(a&&g&&1==h%2){l.copy(b[h]);k.copy(b[h-1]);a.to(l);a.to(k);l.subSelf(k);var s=l.length(),t=Math.min(1,0.5*s/j),t=(1-(1-t)*(1-t))*j;l.normalize().multiplyScalar(s-t).addSelf(k);a.from(l);b[h].copy(l);c[r].set({size:t})}r=m?r+1:r;m=(m+1)%2}.bind(this))}});MathBox.Primitive.types.vector=MathBox.Vector;
...
c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,g=c.flipSided,h=g?1:0,g=g?0:1;"number"==typeof f&&(f=[f,f]);var j=e[0][0],k=e[1][0],l=(e[0][1]-j)/(f[0]-1),m=(e[1][1]-k)/(f[1]-1),r,q=0;_.loop(f[1],function(b){j=e[0][0];_.loop(f[0],function(e){r=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,j,k,e,b,q);r instanceof Array||(r=[j,r,k]);r=r.concat([0,
0,0]);a[q].set.apply(a[q],r);j+=l;q++}.bind(this));k+=m}.bind(this));if(c.shaded){var q=0,k=e[1][0],s=f[0];_.loop(f[1],function(c){j=e[0][0];var d=c?c-1:0,g=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var h=e?e-1:0,k=Math.min(f[0]-1,e+1),r=a[e+c*s];k==e?b[0][q].sub(r,a[h+c*s],r).addSelf(r):b[0][q].copy(a[k+c*s]);g==c?b[1][q].sub(r,a[e+d*s]).addSelf(r):b[1][q].copy(a[e+g*s]);j+=l;q++});k+=m});this.line.set("attributes",{positionDU:b[h],positionDV:b[g]});this.mesh.set("attributes",{positionDU:b[h],
...
this.matrixZ.set(j[0][2],j[1][2],j[2][2],j[3][2],j[4][2],j[5][2],j[6][2],j[7][2],j[8][2],j[9][2],j[10][2],j[11][2],j[12][2],j[13][2],j[14][2],j[15][2]);var k=this.coefficients;this.matrixX.multiplySelf(k).transpose().multiplySelf(k);this.matrixY.multiplySelf(k).transpose().multiplySelf(k);this.matrixZ.multiplySelf(k).transpose().multiplySelf(k);var k={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},l=c[0][0],m=c[1][0],r=(c[0][1]-l)/(g[0]-1),q=(c[1][1]-m)/(g[1]-
1),s=0;_.loop(g[1],function(){l=c[0][0];_.loop(g[0],function(){a[s].set(l,m,0);l+=r;s++}.bind(this));m+=q}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});
...
c[1][1]-e,c=c[2][1]-f,j=d[0],k=d[1],d=d[2],l=[g/(2*j),0,0,b+g/2,0,h/(2*k),0,e+h/2,0,0,c/(2*d),f+c/2,0,0,0,1];this.transform.set.apply(this.transform,[2*j/g,0,0,-(2*b+g)*j/g,0,2*k/h,0,-(2*e+h)*k/h,0,0,2*d/c,-(2*f+c)*d/c,0,0,0,1]);this.inverse.set.apply(this.inverse,l);MathBox.Viewport.prototype.update.call(this,a)},validateRange:function(a){a=a||[];for(var b=0;3>b;++b){a[b]=a[b]||[];for(var c=0;2>c;++c)a[b][c]=void 0!==a[b][c]?a[b][c]:2*c-1}return a},validateScale:function(a){a=a||[];for(var b=0;3>
