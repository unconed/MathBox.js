...
a);if(!g[0])throw"Could not parse shader snippet. Must contain a void-returning function with in/outs: "+a;var j=a;_.each({parseAttribute:d,parseUniform:e,parseVarying:f},function(a,b){_.each(a,function(a){this[b](a);j=j.replace(a[0],"")}.bind(this))}.bind(this));j=j.replace(/^\s*;/,"");this.parseSignature(g[0]);this.body=j}}})(ShaderGraph);
...
")";case "function":return function(){return a(c.apply(this,arguments),d.apply(this,arguments))};case "boolean":return 0.5<g?d:c;case "number":return c+(d-c)*j;case "object":if(d.constructor==Array)return f=[],_.loop(c.length,function(b){f[b]=a(c[b],d[b])}),f;if(d.constructor==THREE.Matrix4){f=new THREE.Matrix4;for(var l=0;16>l;++l)f.elements[l]=c.elements[l]+(d.elements[l]-c.elements[l])*j;return f}if(d.constructor==THREE.Vector3)return f=new THREE.Vector3,f.x=c.x+(d.x-c.x)*j,f.y=c.y+(d.y-c.y)*j,
f.z=c.z+(d.z-c.z)*j,f;if(d.constructor==THREE.Color)return f=new THREE.Color,f.r=c.r+(d.r-c.r)*j,f.g=c.g+(d.g-c.g)*j,f.b=c.b+(d.b-c.b)*j,f;throw"Unimplemented value type in animator. "+typeof d+" ("+d.constructor+")";}}this.start||this.init();var b=this.object,c=this.from,d=this.to,e=this.key,f=this.ease,g;this.fraction=g=0<this.duration?Math.min(1,(MathBox.Animator.now-this.start)/(this.duration||1)):1;var j;switch(f){case "in":j=1-(1-g)*(1-g);break;case "out":j=g*g;break;default:j=0.5-0.5*Math.cos(g*
...
b)}};MathBox.Ticks=function(a,b,c,d,e,f){var g=(b-a)/(c||10),d=d||1,c=d==\u03c0?2:10,j=d*((f||0)+Math.pow(c,Math.floor(Math.log(g/d)/Math.log(c)))),d=_.map(c==\u03c0?[1]:[5,1,0.5],function(a){return j*a}),h=_.reduce(d,function(a,b){return Math.abs(b-g)<Math.abs(a-g)?b:a},j),e=+!e,a=(Math.ceil(a/h)+e)*h,b=(Math.floor(b/h)-e)*h,c=Math.ceil((b-a)/h)+1,k=[];_.loop(c,function(b){k.push(a+b*h)});return k};MathBox.TicksLog=function(){};
...
MathBox.Director.prototype={clock:function(a,b){if(b||!this.clocks[a])this.clocks[a]=+new Date;return 0.001*(+new Date-this.clocks[a])},invert:function(a){var b=this._stage,c=[],d,e=a[0],f=a[1],g=a[2],j=a[3];switch(e){case "add":d=[0];case "clone":d=d||b.select(f);_.each(d,function(a,b){c.push(["remove",g.sequence||MathBox.Primitive.sequence+1+b])});break;case "remove":d=b.select(f);_.each(d,function(a){c.push(["add",a.type(),b.get(a)])});break;case "animate":case "set":d=b.select(f),_.each(d,function(a){var d=
j&&Math.min(300,j.duration)||300;c.push([e,a.singleton||a.get("sequence"),b.get(a),{duration:d}])})}return c},apply:function(a,b,c){var d=this._stage;_.each(a,function(a){var f=a[0]||"",g=a[1]||"",j=a[2]||{},h=a[3]||{};"remove"==f&&(h=j);b&&(a=this.invert(a),a=[0,0].concat(a),Array.prototype.splice.apply(b,a));c&&h&&(h=_.extend({},h),h.delay=0,h.duration=Math.min(300,h.duration));switch(f){case "clone":d.clone(g,j,h);break;case "add":d.spawn(g,j,h);break;case "remove":_.each(d.select(g),function(a){d.remove(a,
h)});break;case "set":var f=d.select(g),k=j.constructor==Array;_.each(f,function(a,b){d.set(a,k?j[b]:j)});break;case "animate":f=d.select(g),k=j.constructor==Array,_.each(f,function(a,b){d.animate(a,k?j[b]:j,h)})}}.bind(this));return this},insert:function(a){a[0].constructor!=Array&&(a=[a]);this.script.splice(this.step,0,a);this.forward();return this},is:function(a){if(!this.script.length)return!1;for(;0>a;)a+=this.script.length+1;for(;a>=this.script.length+1;)a-=this.script.length+1;return a==this.step},
...
c=function(c){return new MathBox.Renderable.Mesh(d,{type:c,dynamic:a.live},b)};this.line=c("line");this.points=c("points");this.calculate()},calculate:function(){var a=this.vertices,b=this.get(),c=b.data,d=b.domain,e=b.n,f=d[0],g=(d[1]-f)/(e-1),j;_.loop(e,function(d){j=c&&void 0!==c[d]?c[d]:b.expression.call(this,f,d);j instanceof Array||(j=[f,j,0]);j=j.concat([0,0,0]);a[d].set.apply(a[d],j);f+=g}.bind(this))}});MathBox.Primitive.types.curve=MathBox.Curve;
MathBox.Bezier=function(a){MathBox.Curve.call(this,a)};
MathBox.Bezier.prototype=_.extend(new MathBox.Curve(null),{defaults:function(){return{n:64,domain:[0,1],data:null,order:3,expression:function(){return 0},live:!0,points:!1,line:!0,style:{}}},type:function(){return"bezier"},calculate:function(){var a=this.vertices,b=this.get(),c=b.domain,d=b.data,e=b.order,f=b.n;if(1>e||3<e)throw"Bezier curve order must be 1, 2 or 3.";var g=[],j;_.loop(e+1,function(a){j=d&&void 0!==d[a]?d[a]:b.expression.call(this,a,a);j instanceof Array||(j=[k,j,0]);j=j.concat([0,
0,0]);g.push(new THREE.Vector3(j[0],j[1],j[2]))}.bind(this));var h={bezierPoints:g},k=c[0],q=(c[1]-k)/(f-1);_.loop(f,function(b){a[b].set(k,0,0);k+=q});c={uniforms:h,shaders:{position:function(a,b){a.snippet("bezier"+e).snippet("mathTransform");b.viewport(a)}}};this.line.set(c);this.points.set(c)}});MathBox.Primitive.types.bezier=MathBox.Bezier;MathBox.Axis=function(a){null!==a&&MathBox.Primitive.call(this,a)};
MathBox.Axis.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{axis:0,offset:[0,0,0],n:2,labels:!1,distance:15,ticks:10,tickBase:1,arrow:!0,size:0.07,style:{lineWidth:4,color:new THREE.Color(7368816)},zero:!0}},renderables:function(){return[this.line,this.ticks,this.arrow,this.labels]},type:function(){return"axis"},adjust:function(a){var b=this.get(),c=b.axis,d=b.offset,e=b.arrow,f=b.labels,g=b.ticks,j=b.tickBase,h=b.n,k=this.points,q=this.labelPoints,b=this.labelTangent,
l=this.tickPoints,r=this.tickSigns,m=[0,0,0],s=new THREE.Vector3;new THREE.Vector4;var a=a.axis(c),t=a[0],a=a[1],v=(a-t)/(h-1);s.set.apply(s,d);_.loop(h,function(a){m[c]=t+a*v;k[a].set.apply(k[a],m);k[a].addSelf(s)});this.arrow.show(e);d=this.scale=MathBox.Ticks(t,a,g,j,!0);this.ticks.show(!!g);if(g){g=Math.floor(l.length/2);d.length>g&&(d=d.slice(0,g));var g=[0,0,0],u;g[2==c?0:1-c]=0.01*(a-t);this.epsilon.set.apply(this.epsilon,g);m=[0,0,0];_.each(d,function(a,b){m[c]=a;var d=2*b;l[d].set.apply(l[d],
m);l[d].addSelf(s);r[d]=1;l[d+1].copy(l[d]);r[d+1]=-1;q[b].copy(l[d]);u=d+1});g=u+1;for(h=l.length;g<h;)l[g].copy(l[u]),r[g]=r[u],g++}m=[0,0,0];m[c]=1;b.set.apply(b,m);this.labels.show(!!f)},make:function(){var a=this.get(),b=a.n,c=a.ticks,d=this.style,e=this.points=[],f=this.labelPoints=[],g=this.labelTangent=new THREE.Vector3,j=this.tickPoints=[],h=this.tickSigns=[],k=this.epsilon=new THREE.Vector3;_.loop(b,function(){e.push(new THREE.Vector3)});_.loop(4*c,function(){f.push(new THREE.Vector3)});
_.loop(8*c,function(){j.push(new THREE.Vector3);h.push(1)});var c={dynamic:!0,size:a.size,offset:0.5},q={dynamic:!0,size:0.2*a.size},l={dynamic:!0,distance:a.distance},r=function(b){b=this.scale[b];return 0==b&&!a.zero?"":b}.bind(this);this.line=new MathBox.Renderable.Mesh(e,{dynamic:!0,type:"line"},d);this.arrow=new MathBox.Renderable.ArrowHead(e[b-2],e[b-1],c,d);this.ticks=new MathBox.Renderable.Ticks(j,h,k,q,d);this.labels=new MathBox.Renderable.Labels(f,g,r,l,d)}});
...
MathBox.Vector.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{n:1,data:null,arrow:!0,expression:function(){return 0},live:!0,style:{},size:0.07}},type:function(){return"vector"},renderables:function(){return this.render},adjust:function(a){var b=this.get();this.calculate(a);_.each(this.arrows,function(a){a.show(b.arrow)})},make:function(){var a=this.get(),b=this.style,c=a.n,d=this.vertices=[],e=this.points=[],f=this.render=[],g=this.arrows=[],j={dynamic:a.live,type:"line",
strip:!1},h={size:a.size},k=0;_.loop(c,function(){d.push(new THREE.Vector3);d.push(new THREE.Vector3);e.push(new THREE.Vector3);e.push(new THREE.Vector3);var a=new MathBox.Renderable.ArrowHead(e[k++],e[k++],h,b);f.push(a);g.push(a)});this.line=new MathBox.Renderable.Mesh(d,j,b);f.push(this.line);this.calculate()},calculate:function(a){var b=this.vertices,c=this.points,d=this.get(),e=d.data,f=d.arrow,g=d.n,j=d.size,h=new THREE.Vector3,k=new THREE.Vector3,q=0,l=0;_.loop(2*g,function(g){p=e&&void 0!==
e[g]?e[g]:d.expression.call(this,l,q);p instanceof Array||(p=[g,p,0]);p=p.concat([0,0,0]);c[g].set.apply(c[g],p);b[g].set.apply(b[g],p);if(a&&f&&1==g%2){k.copy(b[g]);h.copy(b[g-1]);a.to(k);a.to(h);k.subSelf(h);var m=k.length()-j;k.normalize().multiplyScalar(m).addSelf(h);a.from(k);b[g].copy(k)}l=q?l+1:l;q=(q+1)%2}.bind(this))}});MathBox.Primitive.types.vector=MathBox.Vector;MathBox.Surface=function(a){null!==a&&MathBox.Primitive.call(this,a)};
...
c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,g=c.flipSided,j=g?1:0,g=g?0:1;"number"==typeof f&&(f=[f,f]);var h=e[0][0],k=e[1][0],q=(e[0][1]-h)/(f[0]-1),l=(e[1][1]-k)/(f[1]-1),r,m=0;_.loop(f[1],function(b){h=e[0][0];_.loop(f[0],function(e){r=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,h,k,e,b,m);r instanceof Array||(r=[h,r,k]);r=r.concat([0,
0,0]);a[m].set.apply(a[m],r);h+=q;m++}.bind(this));k+=l}.bind(this));if(c.shaded){var m=0,k=e[1][0],s=f[0];_.loop(f[1],function(c){h=e[0][0];var d=c?c-1:0,g=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var j=e?e-1:0,k=Math.min(f[0]-1,e+1),l=a[e+c*s];k==e?b[0][m].sub(l,a[j+c*s],l).addSelf(l):b[0][m].copy(a[k+c*s]);g==c?b[1][m].sub(l,a[e+d*s]).addSelf(l):b[1][m].copy(a[e+g*s]);h+=q;m++});k+=l});this.line.set("attributes",{positionDU:b[j],positionDV:b[g]});this.mesh.set("attributes",{positionDU:b[j],
...
MathBox.BezierSurface.prototype=_.extend(new MathBox.Surface(null),{defaults:function(){return{n:[64,64],domain:[[0,1],[0,1]],data:null,order:3,expression:function(){return 0},live:!0,points:!1,line:!1,mesh:!0,doubleSided:!0,flipSided:!1,shaded:!0,style:{}}},type:function(){return"bezierSurface"},calculate:function(){var a=this.vertices,b=this.get(),c=b.domain,d=b.data,e=b.order,f=b.shaded,g=b.n;"number"==typeof g&&(g=[g,g]);if(3!=e)throw"Bezier surface order must be 3.";var j=[],h;_.loop(e+1,function(a){_.loop(e+
1,function(c){h=d&&void 0!==d[a]&&void 0!==d[a][c]?d[c]:b.expression.call(this,c,a,c,a);h instanceof Array||(h=[c,h,a]);h=h.concat([0,0,0]);j.push(h)}.bind(this))}.bind(this));h=j;this.matrixX.set(h[0][0],h[1][0],h[2][0],h[3][0],h[4][0],h[5][0],h[6][0],h[7][0],h[8][0],h[9][0],h[10][0],h[11][0],h[12][0],h[13][0],h[14][0],h[15][0]);this.matrixY.set(h[0][1],h[1][1],h[2][1],h[3][1],h[4][1],h[5][1],h[6][1],h[7][1],h[8][1],h[9][1],h[10][1],h[11][1],h[12][1],h[13][1],h[14][1],h[15][1]);this.matrixZ.set(h[0][2],
h[1][2],h[2][2],h[3][2],h[4][2],h[5][2],h[6][2],h[7][2],h[8][2],h[9][2],h[10][2],h[11][2],h[12][2],h[13][2],h[14][2],h[15][2]);var k=this.coefficients;this.matrixX.multiplySelf(k).transpose().multiplySelf(k);this.matrixY.multiplySelf(k).transpose().multiplySelf(k);this.matrixZ.multiplySelf(k).transpose().multiplySelf(k);var k={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},q=c[0][0],l=c[1][0],r=(c[0][1]-q)/(g[0]-1),m=(c[1][1]-l)/(g[1]-1),s=0;_.loop(g[1],function(){q=
c[0][0];_.loop(g[0],function(){a[s].set(q,l,0);q+=r;s++}.bind(this));l+=m}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});MathBox.Primitive.types.bezierSurface=MathBox.BezierSurface;
...
MathBox.Renderable.Labels.prototype=_.extend(new MathBox.Renderable(null),{defaults:function(){return{absolute:!0,distance:15,size:1}},make:function(){this.get();var a=this.tangent,b=this.sprites,c=this.points.length;this._anchor=new THREE.Vector3;var d=document.createElement("div"),e=this.object=new MathBox.Sprite(d);d.className="mathbox-labels";_.loop(c,function(){var c=document.createElement("div"),d=document.createElement("div");c.appendChild(d);var j=new MathBox.Sprite(c,a);c.className="mathbox-label";
d.className="mathbox-wrap";d.style.position="relative";d.style.display="inline-block";d.style.left="-50%";d.style.top="-.5em";b.push(j);e.add(j)})},adjust:function(a,b,c,d,e){var f=this.get(),g=this.points,j=this.callback,h=this.style,k=window.MathJax&&MathJax.Hub;_.each(this.sprites,function(b,c){b.position.copy(g[c]);a.to(b.position);e.matrix.multiplyVector3(b.position);b.distance=f.distance;b.element.style.opacity=h.get("opacity");var d="";if(j&&(d=j(c),void 0===d&&(d=""),+d==d)){var m=+d;if(0!=
m)var d=0>m?-1:1,m=Math.abs(m),s=Math.pow(10,1-Math.floor(Math.log(m)/Math.log(10))),d=m=d*Math.round(s*m)/s}b.content!==d&&(m=b.element.children[0],b.content=d,k?(m.innerHTML="\\("+d+"\\)",MathJax.Hub.queue.Push(["Typeset",MathJax.Hub,m])):m.innerHTML=d)});MathBox.Renderable.prototype.adjust.call(this,a)}});MathBox.Viewport=function(a){if(null!==a){var b=this.defaults(),a=_.extend(b,a||{});this.set(a);this.singleton="viewport";this._uniforms={}}};
...
e;a.y=(Math.cos(g)*e-c)/b;a.z+=a.x*f*d}this.transform.multiplyVector3(a)},from:function(a){var b=this._uniforms.polarAspect,c=this._uniforms.polarFocus,d=this._uniforms.polarAlpha,e=this._uniforms.polarFold,f=this._uniforms.polarPower,g=this._uniforms.polarHelix;this.inverse.multiplyVector3(a);if(1E-4<d){var j=a.x,h=a.y*b+c,k=Math.sqrt(j*j+h*h);theta=Math.atan2(j,h);a.x=theta/d;a.y=(k-c)/b;a.z-=a.x*g*d}a.x/=e;a.y=Math.sign(a.y)*Math.pow(Math.abs(a.y),1/f)},axis:function(a){var b=this.super.axis.call(this,
a);min=b[0];max=b[1];alpha=this._uniforms.polarAlpha;aspect=this._uniforms.polarAspect;focus=this._uniforms.polarFocus;1==a&&0<alpha&&(max=Math.max(Math.abs(max),Math.abs(min)),min=Math.max(-focus/aspect+0.001,min));return[min,max]},update:function(a){var b=this.get(),c=b.range,d=b.scale,e=b.polar,f=b.fold,g=b.power,b=b.helix,j=1,h=c[0][0],k=c[1][0],q=c[2][0],l=c[0][1]-h,r=c[1][1]-k,c=c[2][1]-q,m=d[0],s=d[1],d=d[2],j=0<l?1:-1,t=Math.abs(r),t=l+(t*j-l)*e,j=Math.abs(t/m/(r/s)),v=[t/(2*m),0,0,h+l/2,
0,r/(2*s),0,k+r/2,0,0,c/(2*d),q+c/2,0,0,0,1];this.transform.set.apply(this.transform,[2*m/t,0,0,-(2*h+l)*m/l,0,2*s/r,0,-(2*k+r)*s/r,0,0,2*d/c,-(2*q+c)*d/c,0,0,0,1]);this.inverse.set.apply(this.inverse,v);this._uniforms.polarAlpha=e;this._uniforms.polarAspect=j;this._uniforms.polarFold=f;this._uniforms.polarPower=g;this._uniforms.polarFocus=0<e?1/e-1:0;this._uniforms.polarHelix=b;MathBox.Viewport.prototype.update.call(this,a)},shader:function(a){a.snippet("polarPower").snippet("cartesianToPolar").snippet("mathToWorld")},
