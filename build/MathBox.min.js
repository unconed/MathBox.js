...
a);if(!g[0])throw"Could not parse shader snippet. Must contain a void-returning function with in/outs: "+a;var i=a;_.each({parseAttribute:d,parseUniform:e,parseVarying:f},function(a,b){_.each(a,function(a){this[b](a);i=i.replace(a[0],"")}.bind(this))}.bind(this));i=i.replace(/^\s*;/,"");this.parseSignature(g[0]);this.body=i}}})(ShaderGraph);
...
")";case "function":return function(){return a(c.apply(this,arguments),d.apply(this,arguments))};case "boolean":return 0.5<g?d:c;case "number":return c+(d-c)*i;case "object":if(d.constructor==Array)return f=[],_.loop(c.length,function(b){f[b]=a(c[b],d[b])}),f;if(d.constructor==THREE.Matrix4){f=new THREE.Matrix4;for(var k=0;16>k;++k)f.elements[k]=c.elements[k]+(d.elements[k]-c.elements[k])*i;return f}if(d.constructor==THREE.Vector3)return f=new THREE.Vector3,f.x=c.x+(d.x-c.x)*i,f.y=c.y+(d.y-c.y)*i,
f.z=c.z+(d.z-c.z)*i,f;if(d.constructor==THREE.Color)return f=new THREE.Color,f.r=c.r+(d.r-c.r)*i,f.g=c.g+(d.g-c.g)*i,f.b=c.b+(d.b-c.b)*i,f;throw"Unimplemented value type in animator. "+typeof d+" ("+d.constructor+")";}}this.start||this.init();var b=this.object,c=this.from,d=this.to,e=this.key,f=this.ease,g;this.fraction=g=0<this.duration?Math.min(1,(MathBox.Animator.now-this.start)/(this.duration||1)):1;var i;switch(f){case "in":i=1-(1-g)*(1-g);break;case "out":i=g*g;break;default:i=0.5-0.5*Math.cos(g*
...
b)}};MathBox.Ticks=function(a,b,c,d,e,f){var g=(b-a)/(c||10),d=d||1,c=d==\u03c0?2:10,i=d*((f||0)+Math.pow(c,Math.floor(Math.log(g/d)/Math.log(c)))),d=_.map(c==\u03c0?[1]:[5,1,0.5],function(a){return i*a}),h=_.reduce(d,function(a,b){return Math.abs(b-g)<Math.abs(a-g)?b:a},i),e=+!e,a=(Math.ceil(a/h)+e)*h,b=(Math.floor(b/h)-e)*h,c=Math.ceil((b-a)/h)+1,j=[];_.loop(c,function(b){j.push(a+b*h)});return j};MathBox.TicksLog=function(){};
...
MathBox.Director.prototype={clock:function(a,b){if(b||!this.clocks[a])this.clocks[a]=+new Date;return 0.001*(+new Date-this.clocks[a])},invert:function(a){var b=this._stage,c=[],d,e=a[0],f=a[1],g=a[2],i=a[3];switch(e){case "add":d=[0];case "clone":d=d||b.select(f);_.each(d,function(a,b){c.push(["remove",g.sequence||MathBox.Primitive.sequence+1+b])});break;case "remove":d=b.select(f);_.each(d,function(a){c.push(["add",a.type(),b.get(a)])});break;case "animate":case "set":d=b.select(f),_.each(d,function(a){var d=
i&&Math.min(300,i.duration)||300;c.push([e,a.singleton||a.get("sequence"),b.get(a),{duration:d}])})}return c},apply:function(a,b,c){var d=this._stage;_.each(a,function(a){var f=a[0]||"",g=a[1]||"",i=a[2]||{},h=a[3]||{};"remove"==f&&(h=i);b&&(a=this.invert(a),a=[0,0].concat(a),Array.prototype.splice.apply(b,a));c&&h&&(h=_.extend({},h),h.delay=0,h.duration=Math.min(300,h.duration));switch(f){case "clone":d.clone(g,i,h);break;case "add":d.spawn(g,i,h);break;case "remove":_.each(d.select(g),function(a){d.remove(a,
h)});break;case "set":var f=d.select(g),j=i.constructor==Array;_.each(f,function(a,b){d.set(a,j?i[b]:i)});break;case "animate":f=d.select(g),j=i.constructor==Array,_.each(f,function(a,b){d.animate(a,j?i[b]:i,h)})}}.bind(this));return this},insert:function(a){a[0].constructor!=Array&&(a=[a]);this.script.splice(this.step,0,a);this.forward();return this},is:function(a){if(!this.script.length)return!1;for(;0>a;)a+=this.script.length+1;for(;a>=this.script.length+1;)a-=this.script.length+1;return a==this.step},
...
c=function(c){return new MathBox.Renderable.Mesh(d,{type:c,dynamic:a.live},b)};this.line=c("line");this.points=c("points");this.calculate()},calculate:function(){var a=this.vertices,b=this.get(),c=b.data,d=b.domain,e=b.n,f=d[0],g=(d[1]-f)/(e-1),i;_.loop(e,function(d){i=c&&void 0!==c[d]?c[d]:b.expression.call(this,f,d);i instanceof Array||(i=[f,i,0]);i=i.concat([0,0,0]);a[d].set.apply(a[d],i);f+=g}.bind(this))}});MathBox.Primitive.types.curve=MathBox.Curve;
MathBox.Bezier=function(a){MathBox.Curve.call(this,a)};
MathBox.Bezier.prototype=_.extend(new MathBox.Curve(null),{defaults:function(){return{n:64,domain:[0,1],data:null,order:3,expression:function(){return 0},live:!0,points:!1,line:!0,style:{}}},type:function(){return"bezier"},calculate:function(){var a=this.vertices,b=this.get(),c=b.domain,d=b.data,e=b.order,f=b.n;if(1>e||3<e)throw"Bezier curve order must be 1, 2 or 3.";var g=[],i;_.loop(e+1,function(a){i=d&&void 0!==d[a]?d[a]:b.expression.call(this,a,a);i instanceof Array||(i=[j,i,0]);i=i.concat([0,
0,0]);g.push(new THREE.Vector3(i[0],i[1],i[2]))}.bind(this));var h={bezierPoints:g},j=c[0],m=(c[1]-j)/(f-1);_.loop(f,function(b){a[b].set(j,0,0);j+=m});c={uniforms:h,shaders:{position:function(a,b){a.snippet("bezier"+e).snippet("mathTransform");b.viewport(a)}}};this.line.set(c);this.points.set(c)}});MathBox.Primitive.types.bezier=MathBox.Bezier;MathBox.Axis=function(a){null!==a&&MathBox.Primitive.call(this,a)};
MathBox.Axis.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{axis:0,offset:[0,0,0],n:2,labels:!1,distance:15,ticks:10,tickBase:1,arrow:!0,size:0.07,style:{lineWidth:4,color:new THREE.Color(7368816)},zero:!0}},renderables:function(){return[this.line,this.ticks,this.arrow,this.labels]},type:function(){return"axis"},adjust:function(a){var b=this.get(),c=b.axis,d=b.offset,e=b.arrow,f=b.labels,g=b.ticks,i=b.tickBase,h=b.n,j=this.points,m=this.labelPoints,b=this.labelTangent,
k=this.tickPoints,r=this.tickSigns,l=[0,0,0],q=new THREE.Vector3;new THREE.Vector4;var a=a.axis(c),s=a[0],a=a[1],u=(a-s)/(h-1);q.set.apply(q,d);_.loop(h,function(a){l[c]=s+a*u;j[a].set.apply(j[a],l);j[a].addSelf(q)});this.arrow.show(e);d=this.scale=MathBox.Ticks(s,a,g,i,!0);this.ticks.show(!!g);if(g){g=Math.floor(k.length/2);d.length>g&&(d=d.slice(0,g));var g=[0,0,0],t;g[2==c?0:1-c]=0.01*(a-s);this.epsilon.set.apply(this.epsilon,g);l=[0,0,0];_.each(d,function(a,b){l[c]=a;var d=2*b;k[d].set.apply(k[d],
l);k[d].addSelf(q);r[d]=1;k[d+1].copy(k[d]);r[d+1]=-1;m[b].copy(k[d]);t=d+1});g=t+1;for(h=k.length;g<h;)k[g].copy(k[t]),r[g]=r[t],g++}l=[0,0,0];l[c]=1;b.set.apply(b,l);this.labels.show(!!f)},make:function(){var a=this.get(),b=a.n,c=a.ticks,d=this.style,e=this.points=[],f=this.labelPoints=[],g=this.labelTangent=new THREE.Vector3,i=this.tickPoints=[],h=this.tickSigns=[],j=this.epsilon=new THREE.Vector3;_.loop(b,function(){e.push(new THREE.Vector3)});_.loop(4*c,function(){f.push(new THREE.Vector3)});
_.loop(8*c,function(){i.push(new THREE.Vector3);h.push(1)});var c={dynamic:!0,size:a.size,offset:0.5},m={dynamic:!0,size:0.2*a.size},k={dynamic:!0,distance:a.distance},r=function(b){b=this.scale[b];return 0==b&&!a.zero?"":b}.bind(this);this.line=new MathBox.Renderable.Mesh(e,{dynamic:!0,type:"line"},d);this.arrow=new MathBox.Renderable.ArrowHead(e[b-2],e[b-1],c,d);this.ticks=new MathBox.Renderable.Ticks(i,h,j,m,d);this.labels=new MathBox.Renderable.Labels(f,g,r,k,d)}});
...
MathBox.Grid.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{axis:[0,1],offset:[0,0,0],show:[!0,!0],n:2,ticks:[10,10],tickBase:[1,1],style:{lineWidth:1,color:new THREE.Color(10526880)}}},type:function(){return"grid"},renderables:function(){return this.lines||[]},adjust:function(a){function b(a,b,c,d,e){var f=h.slice(),g=0,i=MathBox.Ticks(b.min,b.max,b.ticks,b.tickBase,!0);i.length>d&&(i=i.slice(0,d));_.each(i,function(d){f[b.axis]=d+h[b.axis];f[c.axis]=c.min+h[c.axis];_.loop(e-
1,function(){a[g].set.apply(a[g],f);g++;f[c.axis]+=c.inv;a[g].set.apply(a[g],f);g++})});d=Math.max(0,g-1);for(i=a.length;g<i;)a[g++].copy(a[d])}var c=this.get(),d=c.axis,e=c.ticks,f=c.tickBase,g=c.n,i=c.show,h=c.offset,c=this.points,j=this.limit,m=[];"number"==typeof g&&(g=[g,g]);_.each(d,function(b,c){var d=a.axis(b);m.push({axis:b,min:d[0],max:d[1],inv:(d[1]-d[0])/(g[c]-1),ticks:e[c],tickBase:f[c]})});this.lines[0].show(i[0]);this.lines[1].show(i[1]);i[0]&&b(c[0],m[1],m[0],j[0],g[0]);i[1]&&b(c[1],
m[0],m[1],j[1],g[1])},make:function(){var a=this.get(),b=a.ticks,c=a.n,d=this.points=[[],[]],a=this.style;"number"==typeof c&&(c=[c,c]);var e=this.limit=[0,0];_.each(b,function(a,b){b=1-b;e[b]=4*a;_.loop(2*e[b]*(c[b]-1),function(){d[b].push(new THREE.Vector3)})});this.lines=[];this.lines.push(new MathBox.Renderable.Mesh(d[0],{type:"line",strip:!1,dynamic:!0},a));this.lines.push(new MathBox.Renderable.Mesh(d[1],{type:"line",strip:!1,dynamic:!0},a))}});MathBox.Primitive.types.grid=MathBox.Grid;
...
MathBox.Vector.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{n:1,data:null,arrow:!0,expression:function(){return 0},live:!0,style:{},size:0.07}},type:function(){return"vector"},renderables:function(){return this.render},adjust:function(a){var b=this.get();this.calculate(a);_.each(this.arrows,function(a){a.show(b.arrow)})},make:function(){var a=this.get(),b=this.style,c=a.n,d=this.vertices=[],e=this.points=[],f=this.render=[],g=this.arrows=[],i={dynamic:a.live,type:"line",
strip:!1},h={size:a.size},j=0;_.loop(c,function(){d.push(new THREE.Vector3);d.push(new THREE.Vector3);e.push(new THREE.Vector3);e.push(new THREE.Vector3);var a=new MathBox.Renderable.ArrowHead(e[j++],e[j++],h,b);f.push(a);g.push(a)});this.line=new MathBox.Renderable.Mesh(d,i,b);f.push(this.line);this.calculate()},calculate:function(a){var b=this.vertices,c=this.points,d=this.get(),e=d.data,f=d.arrow,g=d.n,i=d.size,h=new THREE.Vector3,j=new THREE.Vector3,m=0,k=0;_.loop(2*g,function(g){p=e&&void 0!==
e[g]?e[g]:d.expression.call(this,k,m);p instanceof Array||(p=[g,p,0]);p=p.concat([0,0,0]);c[g].set.apply(c[g],p);b[g].set.apply(b[g],p);if(a&&f&&1==g%2){j.copy(b[g]);h.copy(b[g-1]);a.to(j);a.to(h);j.subSelf(h);var l=j.length()-i;j.normalize().multiplyScalar(l).addSelf(h);a.from(j);b[g].copy(j)}k=m?k+1:k;m=(m+1)%2}.bind(this))}});MathBox.Primitive.types.vector=MathBox.Vector;MathBox.Surface=function(a){null!==a&&MathBox.Primitive.call(this,a)};
...
c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,g=c.flipSided,i=g?1:0,g=g?0:1;"number"==typeof f&&(f=[f,f]);var h=e[0][0],j=e[1][0],m=(e[0][1]-h)/(f[0]-1),k=(e[1][1]-j)/(f[1]-1),r,l=0;_.loop(f[1],function(b){h=e[0][0];_.loop(f[0],function(e){r=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,h,j,e,b,l);r instanceof Array||(r=[h,r,j]);r=r.concat([0,
0,0]);a[l].set.apply(a[l],r);h+=m;l++}.bind(this));j+=k}.bind(this));if(c.shaded){var l=0,j=e[1][0],q=f[0];_.loop(f[1],function(c){h=e[0][0];var d=c?c-1:0,g=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var i=e?e-1:0,j=Math.min(f[0]-1,e+1),k=a[e+c*q];j==e?b[0][l].sub(k,a[i+c*q],k).addSelf(k):b[0][l].copy(a[j+c*q]);g==c?b[1][l].sub(k,a[e+d*q]).addSelf(k):b[1][l].copy(a[e+g*q]);h+=m;l++});j+=k});this.line.set("attributes",{positionDU:b[i],positionDV:b[g]});this.mesh.set("attributes",{positionDU:b[i],
...
MathBox.BezierSurface.prototype=_.extend(new MathBox.Surface(null),{defaults:function(){return{n:[64,64],domain:[[0,1],[0,1]],data:null,order:3,expression:function(){return 0},live:!0,points:!1,line:!1,mesh:!0,doubleSided:!0,flipSided:!1,shaded:!0,style:{}}},type:function(){return"bezierSurface"},calculate:function(){var a=this.vertices,b=this.get(),c=b.domain,d=b.data,e=b.order,f=b.shaded,g=b.n;"number"==typeof g&&(g=[g,g]);if(3!=e)throw"Bezier surface order must be 3.";var i=[],h;_.loop(e+1,function(a){_.loop(e+
1,function(c){h=d&&void 0!==d[a]&&void 0!==d[a][c]?d[c]:b.expression.call(this,c,a,c,a);h instanceof Array||(h=[c,h,a]);h=h.concat([0,0,0]);i.push(h)}.bind(this))}.bind(this));h=i;this.matrixX.set(h[0][0],h[1][0],h[2][0],h[3][0],h[4][0],h[5][0],h[6][0],h[7][0],h[8][0],h[9][0],h[10][0],h[11][0],h[12][0],h[13][0],h[14][0],h[15][0]);this.matrixY.set(h[0][1],h[1][1],h[2][1],h[3][1],h[4][1],h[5][1],h[6][1],h[7][1],h[8][1],h[9][1],h[10][1],h[11][1],h[12][1],h[13][1],h[14][1],h[15][1]);this.matrixZ.set(h[0][2],
h[1][2],h[2][2],h[3][2],h[4][2],h[5][2],h[6][2],h[7][2],h[8][2],h[9][2],h[10][2],h[11][2],h[12][2],h[13][2],h[14][2],h[15][2]);var j=this.coefficients;this.matrixX.multiplySelf(j).transpose().multiplySelf(j);this.matrixY.multiplySelf(j).transpose().multiplySelf(j);this.matrixZ.multiplySelf(j).transpose().multiplySelf(j);var j={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},m=c[0][0],k=c[1][0],r=(c[0][1]-m)/(g[0]-1),l=(c[1][1]-k)/(g[1]-1),q=0;_.loop(g[1],function(){m=
c[0][0];_.loop(g[0],function(){a[q].set(m,k,0);m+=r;q++}.bind(this));k+=l}.bind(this));j={uniforms:j,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(j);this.line.set(j);this.points.set(j)}});MathBox.Primitive.types.bezierSurface=MathBox.BezierSurface;
...
MathBox.Renderable.Labels.prototype=_.extend(new MathBox.Renderable(null),{defaults:function(){return{absolute:!0,distance:15,size:1}},make:function(){this.get();var a=this.tangent,b=this.sprites,c=this.points.length;this._anchor=new THREE.Vector3;var d=document.createElement("div"),e=this.object=new MathBox.Sprite(d);d.className="mathbox-labels";_.loop(c,function(){var c=document.createElement("div"),d=document.createElement("div");c.appendChild(d);var i=new MathBox.Sprite(c,a);c.className="mathbox-label";
d.className="mathbox-wrap";d.style.position="relative";d.style.display="inline-block";d.style.left="-50%";d.style.top="-.5em";b.push(i);e.add(i)})},adjust:function(a,b,c,d,e){var f=this.get(),g=this.points,i=this.callback,h=this.style,j=window.MathJax&&MathJax.Hub;_.each(this.sprites,function(b,c){b.position.copy(g[c]);a.to(b.position);e.matrix.multiplyVector3(b.position);b.distance=f.distance;b.element.style.opacity=h.get("opacity");var d="";if(i&&(d=i(c),void 0===d&&(d=""),+d==d)){var l=+d;if(0!=
l)var d=0>l?-1:1,l=Math.abs(l),q=Math.pow(10,1-Math.floor(Math.log(l)/Math.log(10))),d=l=d*Math.round(q*l)/q}b.content!==d&&(l=b.element.children[0],b.content=d,j?(l.innerHTML="\\("+d+"\\)",MathJax.Hub.queue.Push(["Typeset",MathJax.Hub,l])):l.innerHTML=d)});MathBox.Renderable.prototype.adjust.call(this,a)}});MathBox.Viewport=function(a){if(null!==a){var b=this.defaults(),a=_.extend(b,a||{});this.set(a);this.singleton="viewport";this._uniforms={}}};
...
from:function(a){var b=this._uniforms.polarAspect,c=this._uniforms.polarFocus,d=this._uniforms.polarAlpha,e=this._uniforms.polarFold,f=this._uniforms.polarPower;this.inverse.multiplyVector3(a);if(1E-4<d){var g=a.x,i=a.y*b+c,h=Math.sqrt(g*g+i*i);theta=Math.atan2(g,i);a.x=theta/d;a.y=(h-c)/b}a.x/=e;a.y=Math.sign(a.y)*Math.pow(Math.abs(a.y),1/f)},axis:function(a){var b=this.super.axis.call(this,a);min=b[0];max=b[1];alpha=this._uniforms.polarAlpha;aspect=this._uniforms.polarAspect;focus=this._uniforms.polarFocus;
1==a&&0<alpha&&(max=Math.max(Math.abs(max),Math.abs(min)),min=Math.max(-focus/aspect+0.001,min));return[min,max]},update:function(a){var b=this.get(),c=b.range,d=b.scale,e=b.polar,f=b.fold,b=b.power,g=1,i=c[0][0],h=c[1][0],j=c[2][0],m=c[0][1]-i,k=c[1][1]-h,c=c[2][1]-j,r=d[0],l=d[1],d=d[2],g=0<m?1:-1,q=Math.abs(k),q=m+(q*g-m)*e,g=Math.abs(q/r/(k/l)),s=[q/(2*r),0,0,i+m/2,0,k/(2*l),0,h+k/2,0,0,c/(2*d),j+c/2,0,0,0,1];this.transform.set.apply(this.transform,[2*r/q,0,0,-(2*i+m)*r/m,0,2*l/k,0,-(2*h+k)*l/
k,0,0,2*d/c,-(2*j+c)*d/c,0,0,0,1]);this.inverse.set.apply(this.inverse,s);this._uniforms.polarAlpha=e;this._uniforms.polarAspect=g;this._uniforms.polarFold=f;this._uniforms.polarPower=b;this._uniforms.polarFocus=0<e?1/e-1:0;MathBox.Viewport.prototype.update.call(this,a)},shader:function(a){a.snippet("polarPower").snippet("cartesianToPolar").snippet("mathToWorld")},validatePolar:function(a){return Math.max(0,Math.min(1,+a||0))}});MathBox.Attributes.mixin(MathBox.Viewport);
...
MathBox.ViewportSphere.prototype=_.extend(new MathBox.ViewportCartesian(null),{defaults:function(){return{type:"sphere",range:[[-1,1],[-1,1],[-1,1]],sphere:1,scale:[1,1,1],rotation:[0,0,0],position:[0,0,0]}},to:function(a){var b=this._uniforms.sphereAspectX,c=this._uniforms.sphereAspectY,d=this._uniforms.sphereYScale,e=this._uniforms.sphereFocus,f=this._uniforms.sphereAlpha;if(1E-4<f){var g=e+a.z*b,i=a.x*f,d=a.y*f/c*d,f=Math.cos(d)*g;a.x=Math.sin(i)*f;a.y=Math.sin(d)*g*c;a.z=(Math.cos(i)*f-e)/b}this.transform.multiplyVector3(a)},
from:function(a){var b=this._uniforms.sphereAspectX,c=this._uniforms.sphereAspectY,d=this._uniforms.sphereYScale,e=this._uniforms.sphereFocus,f=this._uniforms.sphereAlpha;this.inverse.multiplyVector3(a);if(1E-4<f){var g=a.x,i=a.y/c,h=a.z*b+e,j=Math.sqrt(g*g+i*i+h*h);theta=Math.atan2(i,Math.sqrt(g*g+i*i));phi=Math.atan2(g,h);a.x=theta/f;a.y=phi/f*c/d;a.z=(j-e)/b}},axis:function(a){var b=this.super.axis.call(this,a);min=b[0];max=b[1];alpha=this._uniforms.sphereAlpha;beta=this._uniforms.sphereBeta;aspectX=
this._uniforms.sphereAspectX;aspectY=this._uniforms.sphereAspectY;focus=this._uniforms.sphereFocus;2==a&&0<alpha&&(max=Math.max(Math.abs(max),Math.abs(min)),min=Math.max(-focus/aspectX,min));return[min,max]},update:function(a){var b=this.get(),c=b.range,d=b.scale,b=b.sphere,e=0<b?1/b-1:0,f=c[0][0],g=c[1][0],i=c[2][0],h=c[0][1]-f,j=c[1][1]-g,c=c[2][1]-i,m=d[0],k=d[1],d=d[2],r=h+(c-h)*b,l=j+(c-j)*b,q=c/d;aspectX=r/m/q;aspectY=l/k/q/aspectX;aspectZ=2*(j/h*m/k);yScale=Math.min(aspectY/b,1+(aspectZ-1)*
b);q=[r/(2*m),0,0,f+h/2,0,l/(2*k),0,g+j/2,0,0,c/(2*d),i+c/2,0,0,0,1];this.transform.set.apply(this.transform,[2*m/r,0,0,-(2*f+h)*m/h,0,2*k/l,0,-(2*g+j)*k/j,0,0,2*d/c,-(2*i+c)*d/c,0,0,0,1]);this.inverse.set.apply(this.inverse,q);this._uniforms.sphereAlpha=b;this._uniforms.sphereAspectX=aspectX;this._uniforms.sphereAspectY=aspectY;this._uniforms.sphereYScale=yScale;this._uniforms.sphereFocus=e;MathBox.Viewport.prototype.update.call(this,a)},shader:function(a){a.snippet("cartesianToSphere").snippet("mathToWorld")},
