...
m=b.tickScale,b=b.decimals,l=this.points,s=this.labelPoints,r=this.labelTangent,q=this.tickPoints,t=this.tickSigns,u=[0,0,0],v=new THREE.Vector3;new THREE.Vector4;a=a.axis(c);var w=a[0];a=a[1];var y=(a-w)/(g-1);v.set.apply(v,d);_.loop(g,function(a){u[c]=w+a*y;l[a].set.apply(l[a],u);l[a].addSelf(v)});this.line.show(f);this.arrow.show(e);g=this.scale=MathBox.Ticks(w,a,h,k,m,!0);this.ticks.show(!!h);if(h){h=Math.floor(q.length/2);g.length>h&&(g=g.slice(0,h));var h=[0,0,0],x;h[2==c?0:1-c]=0.01*(a-w);
this.epsilon.set.apply(this.epsilon,h);u=[0,0,0];_.each(g,function(a,b){u[c]=a;var d=2*b;q[d].set.apply(q[d],u);q[d].addSelf(v);t[d]=1;q[d+1].copy(q[d]);t[d+1]=-1;s[b].copy(q[d]);x=d+1});h=x+1;for(g=q.length;h<g;)q[h].copy(q[x]),t[h]=t[x],h++}u=[0,0,0];u[c]=1;r.set.apply(r,u);this.labels.show(!!j);this.labels.set("decimals",b)},make:function(){var a=this.get(),b=a.n,c=a.size,d=a.ticks,e=a.distance,f=a.formatter,j=this.style,g=this.points=[],h=this.labelPoints=[],k=this.labelTangent=new THREE.Vector3,
m=this.tickPoints=[],l=this.tickSigns=[],s=this.epsilon=new THREE.Vector3;_.loop(b,function(){g.push(new THREE.Vector3)});_.loop(4*d,function(){h.push(new THREE.Vector3)});_.loop(8*d,function(){m.push(new THREE.Vector3);l.push(1)});var d={dynamic:!0,size:c,offset:0.5},c={dynamic:!0,size:0.2*c},e={dynamic:!0,distance:e},r=function(b){b=this.scale[b];return 0==b&&!a.zero?"":f?f(b):b}.bind(this);this.line=new MathBox.Renderable.Mesh(g,{dynamic:!0,type:"line"},j);this.arrow=new MathBox.Renderable.ArrowHead(g[b-
2],g[b-1],d,j);this.ticks=new MathBox.Renderable.Ticks(m,l,s,c,j);this.labels=new MathBox.Renderable.Labels(h,k,r,e,j)}});MathBox.Axis.validateArgs=function(a){"number"==typeof a&&(a={axis:a});return a};MathBox.Primitive.types.axis=MathBox.Axis;MathBox.Grid=function(a){null!==a&&MathBox.Primitive.call(this,a)};
...
s=0,r=0;_.loop(2*g,function(g){p=f&&void 0!==f[g]?f[g]:e.expression.call(this,r,s);p instanceof Array||(p=[g,p,0]);p=p.concat([0,0,0]);d[g].set.apply(d[g],p);b[g].set.apply(b[g],p);if(a&&j&&1==g%2){l.copy(b[g]);m.copy(b[g-1]);a.to(l);a.to(m);l.subSelf(m).multiplySelf(k);var t=l.length(),u=Math.min(1,0.5*t/h),u=(1-(1-u)*(1-u))*h;l.normalize().multiplyScalar(t-u).divideSelf(k).addSelf(m);a.from(l);b[g].copy(l);c[r].set({size:u})}r=s?r+1:r;s=(s+1)%2}.bind(this))}});MathBox.Primitive.types.vector=MathBox.Vector;
...
[[],[]];_.loop(c[0]*c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,j=c.flipSided,g=j?1:0,j=j?0:1;"number"==typeof f&&(f=[f,f]);var h=e[0][0],k=e[1][0],m=(e[0][1]-h)/(f[0]-1),l=(e[1][1]-k)/(f[1]-1),s,r=0;_.loop(f[1],function(b){h=e[0][0];_.loop(f[0],function(e){s=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,h,k,e,b,r);s instanceof Array||
(s=[h,s,k]);s=s.concat([0,0,0]);a[r].set.apply(a[r],s);h+=m;r++}.bind(this));k+=l}.bind(this));if(c.shaded){var r=0,k=e[1][0],q=f[0];_.loop(f[1],function(c){h=e[0][0];var d=c?c-1:0,g=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var j=e?e-1:0,k=Math.min(f[0]-1,e+1),l=a[e+c*q];k==e?b[0][r].sub(l,a[j+c*q]).addSelf(l):b[0][r].copy(a[k+c*q]);g==c?b[1][r].sub(l,a[e+d*q]).addSelf(l):b[1][r].copy(a[e+g*q]);h+=m;r++});k+=l});this.line.set("attributes",{positionDU:b[g],positionDV:b[j]});this.mesh.set("attributes",
...
this.matrixZ.set(h[0][2],h[1][2],h[2][2],h[3][2],h[4][2],h[5][2],h[6][2],h[7][2],h[8][2],h[9][2],h[10][2],h[11][2],h[12][2],h[13][2],h[14][2],h[15][2]);var k=this.coefficients;this.matrixX.multiplySelf(k).transpose().multiplySelf(k);this.matrixY.multiplySelf(k).transpose().multiplySelf(k);this.matrixZ.multiplySelf(k).transpose().multiplySelf(k);var k={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},m=c[0][0],l=c[1][0],s=(c[0][1]-m)/(j[0]-1),r=(c[1][1]-l)/(j[1]-
1),q=0;_.loop(j[1],function(){m=c[0][0];_.loop(j[0],function(){a[q].set(m,l,0);m+=s;q++}.bind(this));l+=r}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});
