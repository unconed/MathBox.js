...
ThreeRTT.Display.prototype=_.extend(new THREE.Object3D,{make:function(){for(var a=this.n,b=this.gx,c=this.gy,d=this.targets,e=(b-1)/2,f=(c-1)/2,g=new THREE.PlaneGeometry(1,1,1,1),h=0,j=0;h<a&&j<c;++j)for(var k=0;h<a&&k<b;++k,++h){var m=new THREE.MeshBasicMaterial({color:16777215,map:ThreeRTT.toTexture(d[h]),fog:!1});m.side=THREE.DoubleSide;m=new THREE.Mesh(g,m);m.renderDepth=1E4+Math.random();this.add(m);1<b&&(m.position.x=-e+k);1<c&&(m.position.y=f-j)}}});
...
1>b.opacity))},viewport:function(a,b){this.stage.viewport().shader(a,b)}};MathBox.Ticks=function(a,b,c,d,e,f){var g=(b-a)/(c||10);d=d||1;c=d==\u03c0?2:10;var h=d*((f||0)+Math.pow(c,Math.floor(Math.log(g/d)/Math.log(c))));d=_.map(c==\u03c0?[1]:[5,1,0.5],function(a){return h*a});var j=_.reduce(d,function(a,b){return Math.abs(b-g)<Math.abs(a-g)?b:a},h);e=+!e;a=(Math.ceil(a/j)+e)*j;b=(Math.floor(b/j)-e)*j;c=Math.ceil((b-a)/j)+1;var k=[];_.loop(c,function(b){k.push(a+b*j)});return k};
...
0,0]);g.push(new THREE.Vector3(h[0],h[1],h[2]))}.bind(this));var j={bezierPoints:g},k=c[0],m=(c[1]-k)/(f-1);_.loop(f,function(b){a[b].set(k,0,0);k+=m});c={uniforms:j,shaders:{position:function(a,b){a.snippet("bezier"+e).snippet("mathTransform");b.viewport(a)}}};this.line.set(c);this.points.set(c)}});MathBox.Primitive.types.bezier=MathBox.Bezier;MathBox.Axis=function(a){null!==a&&MathBox.Primitive.call(this,a)};
MathBox.Axis.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{axis:0,offset:[0,0,0],n:2,labels:!1,distance:15,ticks:10,tickBase:1,arrow:!0,size:0.07,style:{lineWidth:4,color:new THREE.Color(7368816)},zero:!0}},renderables:function(){return[this.line,this.ticks,this.arrow,this.labels]},type:function(){return"axis"},adjust:function(a){var b=this.get(),c=b.axis,d=b.offset,e=b.arrow,f=b.labels,g=b.ticks,h=b.tickBase,j=b.n,k=this.points,m=this.labelPoints,b=this.labelTangent,
l=this.tickPoints,r=this.tickSigns,q=[0,0,0],s=new THREE.Vector3;new THREE.Vector4;a=a.axis(c);var t=a[0];a=a[1];var v=(a-t)/(j-1);s.set.apply(s,d);_.loop(j,function(a){q[c]=t+a*v;k[a].set.apply(k[a],q);k[a].addSelf(s)});this.arrow.show(e);d=this.scale=MathBox.Ticks(t,a,g,h,!0);this.ticks.show(!!g);if(g){g=Math.floor(l.length/2);d.length>g&&(d=d.slice(0,g));var g=[0,0,0],u;g[2==c?0:1-c]=0.01*(a-t);this.epsilon.set.apply(this.epsilon,g);q=[0,0,0];_.each(d,function(a,b){q[c]=a;var d=2*b;l[d].set.apply(l[d],
q);l[d].addSelf(s);r[d]=1;l[d+1].copy(l[d]);r[d+1]=-1;m[b].copy(l[d]);u=d+1});g=u+1;for(j=l.length;g<j;)l[g].copy(l[u]),r[g]=r[u],g++}q=[0,0,0];q[c]=1;b.set.apply(b,q);this.labels.show(!!f)},make:function(){var a=this.get(),b=a.n,c=a.ticks,d=this.style,e=this.points=[],f=this.labelPoints=[],g=this.labelTangent=new THREE.Vector3,h=this.tickPoints=[],j=this.tickSigns=[],k=this.epsilon=new THREE.Vector3;_.loop(b,function(){e.push(new THREE.Vector3)});_.loop(4*c,function(){f.push(new THREE.Vector3)});
_.loop(8*c,function(){h.push(new THREE.Vector3);j.push(1)});var c={dynamic:!0,size:a.size,offset:0.5},m={dynamic:!0,size:0.2*a.size},l={dynamic:!0,distance:a.distance},r=function(b){b=this.scale[b];return 0==b&&!a.zero?"":b}.bind(this);this.line=new MathBox.Renderable.Mesh(e,{dynamic:!0,type:"line"},d);this.arrow=new MathBox.Renderable.ArrowHead(e[b-2],e[b-1],c,d);this.ticks=new MathBox.Renderable.Ticks(h,j,k,m,d);this.labels=new MathBox.Renderable.Labels(f,g,r,l,d)}});
...
1,function(){a[g].set.apply(a[g],f);g++;f[c.axis]+=c.inv;a[g].set.apply(a[g],f);g++})});d=Math.max(0,g-1);for(h=a.length;g<h;)a[g++].copy(a[d])}var c=this.get(),d=c.axis,e=c.ticks,f=c.tickBase,g=c.n,h=c.show,j=c.offset,c=this.points,k=this.limit,m=[];"number"==typeof g&&(g=[g,g]);_.each(d,function(b,c){var d=a.axis(b);m.push({axis:b,min:d[0],max:d[1],inv:(d[1]-d[0])/(g[c]-1),ticks:e[c],tickBase:f[c]})});this.lines[0].show(h[0]);this.lines[1].show(h[1]);h[0]&&b(c[0],m[1],m[0],k[0],g[0]);h[1]&&b(c[1],
m[0],m[1],k[1],g[1])},make:function(){var a=this.get(),b=a.ticks,c=a.n,d=this.points=[[],[]],a=this.style;"number"==typeof c&&(c=[c,c]);var e=this.limit=[0,0];_.each(b,function(a,b){b=1-b;e[b]=4*a;_.loop(2*e[b]*(c[b]-1),function(){d[b].push(new THREE.Vector3)})});this.lines=[];this.lines.push(new MathBox.Renderable.Mesh(d[0],{type:"line",strip:!1,dynamic:!0},a));this.lines.push(new MathBox.Renderable.Mesh(d[1],{type:"line",strip:!1,dynamic:!0},a))}});MathBox.Primitive.types.grid=MathBox.Grid;
...
strip:!1},j={size:a.size},k=0;_.loop(c,function(){d.push(new THREE.Vector3);d.push(new THREE.Vector3);e.push(new THREE.Vector3);e.push(new THREE.Vector3);var a=new MathBox.Renderable.ArrowHead(e[k++],e[k++],j,b);f.push(a);g.push(a)});this.line=new MathBox.Renderable.Mesh(d,h,b);f.push(this.line);this.calculate()},calculate:function(a){var b=this.vertices,c=this.arrows,d=this.points,e=this.get(),f=e.data,g=e.arrow,h=e.n,j=e.size,k=new THREE.Vector3,m=new THREE.Vector3,l=0,r=0;_.loop(2*h,function(h){p=
f&&void 0!==f[h]?f[h]:e.expression.call(this,r,l);p instanceof Array||(p=[h,p,0]);p=p.concat([0,0,0]);d[h].set.apply(d[h],p);b[h].set.apply(b[h],p);if(a&&g&&1==h%2){m.copy(b[h]);k.copy(b[h-1]);a.to(m);a.to(k);m.subSelf(k);var s=m.length(),t=Math.min(1,0.5*s/j),t=(1-(1-t)*(1-t))*j;m.normalize().multiplyScalar(s-t).addSelf(k);a.from(m);b[h].copy(m);c[r].set({size:t})}r=l?r+1:r;l=(l+1)%2}.bind(this))}});MathBox.Primitive.types.vector=MathBox.Vector;
...
c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,g=c.flipSided,h=g?1:0,g=g?0:1;"number"==typeof f&&(f=[f,f]);var j=e[0][0],k=e[1][0],m=(e[0][1]-j)/(f[0]-1),l=(e[1][1]-k)/(f[1]-1),r,q=0;_.loop(f[1],function(b){j=e[0][0];_.loop(f[0],function(e){r=d&&void 0!==d[b]&&void 0!==d[b][e]?d[b][e]:c.expression.call(this,j,k,e,b,q);r instanceof Array||(r=[j,r,k]);r=r.concat([0,
0,0]);a[q].set.apply(a[q],r);j+=m;q++}.bind(this));k+=l}.bind(this));if(c.shaded){var q=0,k=e[1][0],s=f[0];_.loop(f[1],function(c){j=e[0][0];var d=c?c-1:0,g=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var h=e?e-1:0,k=Math.min(f[0]-1,e+1),l=a[e+c*s];k==e?b[0][q].sub(l,a[h+c*s],l).addSelf(l):b[0][q].copy(a[k+c*s]);g==c?b[1][q].sub(l,a[e+d*s]).addSelf(l):b[1][q].copy(a[e+g*s]);j+=m;q++});k+=l});this.line.set("attributes",{positionDU:b[h],positionDV:b[g]});this.mesh.set("attributes",{positionDU:b[h],
...
this.matrixZ.set(j[0][2],j[1][2],j[2][2],j[3][2],j[4][2],j[5][2],j[6][2],j[7][2],j[8][2],j[9][2],j[10][2],j[11][2],j[12][2],j[13][2],j[14][2],j[15][2]);var k=this.coefficients;this.matrixX.multiplySelf(k).transpose().multiplySelf(k);this.matrixY.multiplySelf(k).transpose().multiplySelf(k);this.matrixZ.multiplySelf(k).transpose().multiplySelf(k);var k={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},m=c[0][0],l=c[1][0],r=(c[0][1]-m)/(g[0]-1),q=(c[1][1]-l)/(g[1]-
1),s=0;_.loop(g[1],function(){m=c[0][0];_.loop(g[0],function(){a[s].set(m,l,0);m+=r;s++}.bind(this));l+=q}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});
...
d.className="mathbox-wrap";d.style.position="relative";d.style.display="inline-block";d.style.left="-50%";d.style.top="-.5em";b.push(h);e.add(h)})},adjust:function(a,b,c,d,e){var f=this.get(),g=this.points,h=this.callback,j=this.style,k=window.MathJax&&MathJax.Hub;_.each(this.sprites,function(b,c){b.position.copy(g[c]);a.to(b.position);e.matrix.multiplyVector3(b.position);b.distance=f.distance;b.element.style.opacity=j.get("opacity");var d="";if(h&&(d=h(c),void 0===d&&(d=""),+d==d)){var q=+d;if(0!=
q)var d=0>q?-1:1,q=Math.abs(q),s=Math.pow(10,1-Math.floor(Math.log(q)/Math.log(10))),d=q=d*Math.round(s*q)/s}b.content!==d&&(q=b.element.children[0],b.content=d,k?(q.innerHTML="\\("+d+"\\)",MathJax.Hub.queue.Push(["Typeset",MathJax.Hub,q])):q.innerHTML=d)});MathBox.Renderable.prototype.adjust.call(this,a)}});MathBox.Viewport=function(a){if(null!==a){var b=this.defaults();a=_.extend(b,a||{});this.set(a);this.singleton="viewport";this._uniforms={}}};
